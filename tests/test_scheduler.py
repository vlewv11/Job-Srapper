...
from src.scraping import scheduler, telegram


def test_cycle_returns_counts(monkeypatch):
    seen = {}

    async def fake_scrape(days=None, full=False, verbose=True):
        seen.update(days=days, full=full)
        return {"channels": 1, "matched": 2, "messages": 1, "pruned": 3,
                "scanned": 4, "topic_skipped": 0, "promo_skipped": 0}
...
def test_cycle_survives_a_telegram_failure(monkeypatch, capsys):
    async def boom(**kwargs):
        raise ConnectionError("telegram unreachable")

    monkeypatch.setattr(telegram, "scrape", boom)
